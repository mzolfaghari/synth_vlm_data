...
def process_single_image(args):
    filename, image_dir, client_cfg, output_file, file_lock, counter, k, print_intermediate = args

    # Build the LLM client (our Qwen vLLM endpoint by default, or Gemini) — see backends.py
    client = make_client(**client_cfg)

    image_path = os.path.join(image_dir, filename)

    try:
        result = generate_questions(client, image_path, k=k)
        ...
        if result:
            ...
            while len(verified_questions) < 2 and verification_attempts < max_verification_attempts:
                ...
                if len(verified_questions) < 2:
                    ...
                    additional_result = generate_questions(client, image_path, k=k)
